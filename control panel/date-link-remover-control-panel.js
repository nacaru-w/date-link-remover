...
const dateLinkeRemoverControlPanel = (() => {
    ...
    const titleRegex = /^((?:\d{1,2}º? de |Anexo:[A-Za-zÀ-ÖØ-öø-ÿ ]+ en )?(?:(?:enero|febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|octubre|noviembre|diciembre)?(?:(?: de )?[1-9]\d{0,3})?)|(?:(?:Anexo:)?(Cronología|Día) de[A-Za-zÀ-ÖØ-öø-ÿ ]+)|(?:lunes|martes|miércoles|jueves|viernes|sábado|domingo)|(?:(?:años?|década de)\s)?(?:[1-9]\d{0,3}(?: \(desambiguación\))?|siglo(?:\s|&nbsp;)*\w+)(?:(?:\s|&nbsp;)*(?:a|d)\.(?:\s|&nbsp;)*C\.)?)$/i;
